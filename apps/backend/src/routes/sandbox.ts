...
export async function sandboxRoutes(fastify: FastifyInstance) {
  ...
  // Kill process
  fastify.post(
    '/sandbox/kill-process',
    {
      schema: {
        body: KillProcessSchema,
        response: {
          200: z.object({
            success: z.boolean(),
            message: z.string(),
  ...
  // Health check
  fastify.get('/sandbox/health', async (request, reply) => {
    try {
